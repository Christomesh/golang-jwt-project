...
func VerifyPassword(userPassoword string, providedPassword string) (bool, string) {
	err := bcrypt.CompareHashAndPassword([]byte(providedPassword), []byte(userPassoword))
	check := true
	msg := ""
	if err != nil {
...
func Signup() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		password := HashPassword(*user.Pasword)
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while checkin for the phone"})
		}
		if count > 0 {
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		passwordIsValid, msg := VerifyPassword(*user.Pasword, *foundUser.Pasword)
		defer cancel()
